...
<%
    ...
    String[] queries = new String[] {
        //firstdigit
        "SELECT {FirstDigit({[element].children})} on rows,"+nl+
            "{[Document].[10-Q]} on columns"+nl+
            "FROM [XbrlDataMart]"+nl+
            "WHERE ([entity].[facebook])",
    	//gri1
    	"SELECT {GriConformity({[element].children})} on rows,"+nl+
		"{[Document].[G4 report],[Document].[10-Q]} on columns"+nl+
		"FROM [XbrlDataMart]"+nl+
		"WHERE ([entity].[hkpc])",
    	//Gri2
    	"with Member [Measures].[Gri]"+nl+
		"  as 'Gri([element].children)'"+nl+
		"select"+nl+
		"  {[Measures].[Gri]} on 0,"+nl+
		"  non empty [Document].children on 1"+nl+
		"from [XbrlDataMart]"+nl+
  		"where [entity].[facebook]",
    		
    	//forensic
    	"select [entity].[facebook] on columns,"+nl+
    	"non empty [element].children on rows"+nl+   
		"from [XbrlDataMart]"+nl+
...
<title>JSP Page (Marcio)</title>
</head>
<body>


<a href=".">back to index (Teste)</a><p/>
        ...
        <tr>
            <td>
                <input type="submit" value="process MDX query">
                 &nbsp; &nbsp; Database:
        		<input type="radio" name="database" value="REL" checked="checked" >Relational
        		<input type="radio" name="database" value="XML" >XML
            </td>
        </tr>
        </table>
