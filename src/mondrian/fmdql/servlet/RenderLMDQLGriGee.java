 ...
 */
public class RenderLMDQLGriGee extends RenderFMDQL implements IRenderFMDQL {
...
public StringBuffer renderResultado(StringBuffer html) {
    ...
    if (this.getResultado().getAxes().length > 1) {        	
            ...
            for (int i=0; i<rows.size(); i++) {
                ...
                for (int j=0; j<columns.size(); j++) {
                    Cell cell = this.getResultado().getCell(new int[]{j,i});                
                    showCellGriGee(html, cell, memberName);
                }
	...
	private void showCellGriGee(StringBuffer out, Cell cell, String ele) {
            String flag = "";
            if(cell.getFormattedValue().trim().equals("")) {
                    ...
                    flag += " GRI Gee: "+formatter.format(this.getGriGeeCalc(
                                    totalUsedElements,
                                    //oj,
                                    totalGriElements,
	...
	private String getElementName(String ele){
        ...
        private double getGriGeeCalc(int a, int c, int d){
            /*
